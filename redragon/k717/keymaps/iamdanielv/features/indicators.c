...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (IS_LAYER_ON(KBCTL_LYR)) {
        ...
        const uint8_t accent_key_indexes[3] = {P_KI,  // P for persistent color
                                               N_KI,  // N for NKRO
                                               FN_KI, // fn key
        };
        set_keys_color((rgb_t){RGB_WHITE}, led_min, led_max, accent_key_indexes, 3 );
        ...
        // layer lock key
        RGB_MATRIX_INDICATOR_SET_COLOR(RIGHT_CTL_KI, 0xAA, 0x22, 0x00);
    ...
    if (IS_LAYER_ON(NUM_LYR)) {
        ...
        // layer toggle
        RGB_MATRIX_INDICATOR_SET_COLOR(RIGHT_MENU_KI, 0xAA, 0x22, 0x00);
    ...
    if (IS_LAYER_ON(ARROW_LYR)) {
        ...
        set_layer_color_indicator(ARROW_LYR, LEFT_WIN_KI, accent_lyr_rgb, led_min, led_max);
    }
